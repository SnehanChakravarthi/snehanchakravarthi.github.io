...
const useBlink = (
	avatarMesh: RefObject<
		SkinnedMesh<BufferGeometry<NormalBufferAttributes>, Material | Material[], Object3DEventMap>
	>
) => {
	...
	useFrame((state) => {
		...
		if (avatarMesh.current?.morphTargetInfluences && avatarMesh.current.morphTargetDictionary) {
			const eyesClosedIndex = avatarMesh.current.morphTargetDictionary['eyesClosed'];
...
function calculateBlinkInfluence(cycleTime: number, timings: any) {
	...
	if (cycleTime < timings.blinkUpTime) {
		...
		influence = 1;
	} else if (cycleTime < timings.blinkUpTime + timings.blinkHoldTime + timings.blinkDownTime) {
		influence =
			1 - (cycleTime - timings.blinkUpTime - timings.blinkHoldTime) / timings.blinkDownTime;
	}
	return influence;
}
